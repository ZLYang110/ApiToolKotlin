...
open class App : Application() , Application.ActivityLifecycleCallbacks  {
    ...
    companion object {
        ...
        fun getApp(): Application  {
            return application ?: throw NullPointerException("App is not registered in the manifest")
        }
    ...
    open fun isAppOnForeground(): Boolean {
        val activityManager =
             getAppContext().getSystemService(ACTIVITY_SERVICE) as ActivityManager
                ?: return false
        val appProcesses = activityManager.runningAppProcesses ?: return false
        val packageName: String =  getAppContext().getPackageName()
    ...
    fun currentActivity(): Activity? {
        return if ( activities == null ||  activities.isEmpty()) {
            null
     ...
     * 按照指定类名找到activity
     */
    open fun findActivity(cls: Class<*>?): Activity? {
        if (cls == null) {
            return null
        }
    ...
    open fun finishActivityWithoutCount(count: Int) {
        if ( activities == null ||  activities.isEmpty()) {
            return
        }
        if (count <= 0) {
     ...
     * 结束指定类名的Activity
     */
    open fun finishActivity(cls: Class<out Activity?>?) {
        if (cls == null) {
            return
        }
        if ( activities == null ||  activities.isEmpty()) {
