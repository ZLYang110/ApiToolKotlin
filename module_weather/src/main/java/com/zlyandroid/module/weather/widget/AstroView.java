...
//		setLayerType(View.LAYER_TYPE_SOFTWARE, null);
	...
	protected void onDraw(Canvas canvas) {
		...
		try {
			...
			canvas.drawText("20" + "km/h " + "北风", fanHeight + textSize, 0, paint);
			...
			canvas.drawText("气压 " + "1006" + "hpa", pressureTextRight, sunArcHeight + paintTextOffset, paint);
			// draw astor info
			final float textLeft = width / 2f - sunArcRadius;// sunArcSize;
			paint.setTextAlign(Align.CENTER);
			canvas.drawText("日出 " + "05:27", textLeft, textSize * 10.5f + paintTextOffset, paint);
			canvas.drawText("19:09" + " 日落", width - textLeft, textSize * 10.5f + paintTextOffset, paint);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
